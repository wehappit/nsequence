...
class TestNSequenceMethods(unittest.TestCase):
    ...
    def test__count_terms_between(self):
        ...
        self.assertEqual(
            self.invertible_sequence.count_terms_between(4, 4), 1
        )

        self.assertEqual(
            self.invertible_sequence.count_terms_between(1, 4), 4
        )

        # Ensure we get correct result when parameters a not passed ordered
        self.assertEqual(
            self.invertible_sequence.count_terms_between(4, 3), 2
        )
    ...
    def test__count_terms_between_terms(self):
        ...
        # 30**4 + 9 = 810009
        self.assertEqual(
            self.invertible_sequence.count_terms_between_terms(
                25,
                25
            ),
            1
    ...
    def test__terms_between(self):
        self.assertEqual(
            self.invertible_sequence.terms_between(
                2, 2
            ),
            [25]
        )
    ...
    def test__terms_between_terms(self):
        ...
        self.assertEqual(
            self.invertible_sequence.terms_between_terms(
                90, 90
            ),
            [90]
        )
        ...
        self.assertEqual(
            ...
            [90, 265, 634, 1305, 2410, 4105, 6570, 10009, 14650, 20745, 28570, 38425, 50634, 65545, 83530, 104985,
             130330, 160009, 194490, 234265, 279850, 331785, 390634, 456985, 531450, 614665, 707290, 810009]
        )
    ...
    def test__nearest_term_position(self):
        ...
        self.assertEqual(self.invertible_sequence.nearest_term_position(707290 + 10), 29)
        ...
        self.assertEqual(self.invertible_sequence.nearest_term_position((25 + 90) / 2, prefer_left_term=True), 2)
        self.assertEqual(self.invertible_sequence.nearest_term_position((25 + 90) / 2, prefer_left_term=False), 3)
...
class TestExceptionRaising(unittest.TestCase):
    def test_should_raise_exception_if_inversion_gives_non_zero_decimal_float(self):
        ...
        self.assertTrue('Expect `inverse_fun` to give integers as results but got a float' in context.exception.msg)
    ...
    def test_should_raise_exception_if_inverse_fun_is_not_callable(self):
        ...
        self.assertTrue('Expect `inverse_func` to be defined and to be callable' in context.exception.msg)
    ...
    def test_should_raise_exception_if_sum_up_nth_term_gets_bad_param(self):
        ...
        self.assertTrue('Expect position to be at least equal to `1`' in context.exception.msg)
...
class TestKwargs(unittest.TestCase):
    ...
    def test_should_sum_up_using_the_provided_sum_up_func(self):
        ...
        # 29 + 1 = 30
        self.assertEqual(
            sequence.sum_up_to_nth_term(29), 30
        )
